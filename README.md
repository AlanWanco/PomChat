...
- Windows 安装版注册 `.pomchat` 文件关联；macOS 安装到“应用程序”后可通过 Finder 打开项目。
- 应用未启动时会启动并加载文件；已启动时会唤起现有窗口。切换项目仍遵循未保存修改的保存、放弃或取消流程。
- 文件关联需要重新打包并安装后生效。若系统已有其他默认程序，可通过“打开方式”选择 PomChat。
- Windows ZIP 便携版不自动注册关联；Linux 需要桌面集成。网页端不支持系统文件关联。
- 仅关联 `.pomchat`，不会占用通用 `.json` 文件的默认打开方式。
