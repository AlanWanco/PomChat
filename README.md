...
- 桌面端可配置 biliup，用于扫码 / 短信登录、投稿模板管理、真实上传进度显示和导出后自动投稿
...
- 在全局设置或“导出 → 设置 biliup”中添加账户并填写各自的 biliup 解压目录，然后检测二进制和 Cookie
- 每个账户对应一个 biliup 目录，切换账户即可切换该目录中的 `cookies.json`
- 支持扫码登录；短信登录会在界面中接收国家代码、手机号和验证码，遇到滑块风控时应改用扫码登录
- 投稿模板保存在本机，可设置标题、标签、分区、转载来源、简介、封面、线路、并发数等 biliup 参数；定时发布时间使用日期时间选择器，限制为当前时间 4 小时至 15 天内
- 导出窗口可启用自动上传；只有保存过有效模板后才能开启，视频成功导出后才会投稿
- biliup 在当前账户目录中运行并自动读取其中的 Cookie；登录凭据不会写入项目文件或上传日志
- 上传日志使用伪终端捕获 biliup 的字节进度，投稿标题超过 80 字时自动截断
