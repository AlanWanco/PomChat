...
- 优化聊天渲染性能：预览与导出均改为时间窗虚拟渲染，减少长对话场景下的渲染负担
- 优化项目设置文案与行为认知：将“顶部范围限制”明确为“聊天顶部留白（Top chat inset）”，补充说明避免误解
- Windows 安装包体验优化：NSIS 安装器支持选择安装路径（非 one-click）
- 发布资产命名优化：移除重复平台后缀拼接，避免 `win-windows-x64` / `mac-macos-arm64` 等冗余命名
